...
try:
    gatewayOptions = {"org": "jh5yoi", "type": "RaspberryPi", "id": "b827ebbeaccc", "auth-method": "token", "auth-token": "Shivansh"}
    gatewayCli = ibmiotf.gateway.Client(gatewayOptions)

    gatewayCli.connect()
    gatewayCli.deviceCommandCallback = myCommandCallback
    gatewayCli.subscribeToDeviceCommands(deviceType='RaspberryPi', deviceId='b827ebbeaccc', command='display',format='json',qos=2)

    while True:
        temp = sense.get_temperature() # TODO insert your code here
        myData = {'d':{'temperature':temp}} # TODO insert your code here
        gatewayCli.publishDeviceEvent("RaspberryPi", "b827ebbeaccc", "yourEventName", "json", myData, qos=1 )
        time.sleep(2)
